// 验证末药「二重治疗」UI 展示：不显示技能期 HPS，显示周期 HPS / 总治疗量
...
state.slots[0] = { operatorId: 'char_117_myrrh', elite: 2, level: 70, trustPercent: 100, potentialRank: 0, skillIndex: 0, skillLevel: 9 };
...
console.log('周期 HPS:', cycle, '(期望 261)');
console.log('总治疗量:', total, '(期望 656)');
console.log('常态 HPS:', normal, '(期望 184)');
console.log('不显示技能期 HPS:', !hasSkillHps ? 'OK' : 'FAIL');

const ok = cycle === '261' && total === '656' && normal === '184' && !hasSkillHps;
console.log('\n最终:', ok ? '通过' : '失败');
process.exit(ok ? 0 : 1);
