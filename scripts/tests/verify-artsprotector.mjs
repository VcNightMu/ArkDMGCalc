...
const hS2 = sk(hs, 1);
check('斩业S2 触发三连击=0.75atk法伤×3', near(hS2.skillTotalDamage, calcArtsDamage(hPs.panelAtk * 0.75, 50) * 3));
check('斩业S2 有循环DPS(攻回sp7周期)', hS2.cycleDps !== null && hS2.cycleDps > 0);
check('斩业S2 damageType=arts(技能触发法术)', hS2.damageType === 'arts');
const hS3 = sk(hs, 2);
