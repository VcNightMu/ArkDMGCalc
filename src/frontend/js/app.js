...
async function updateResults() {
  ...
  for (const slotData of filledSlots) {
    ...
    let metricsHtml = '';
    if (result.type === 'heal') {
      // Medical operator: show healing metrics
      ...
      if (result.skillDps > 0) {
        // Incantation medic also deals damage
...
function calculateOperator(op, slotData) {
  ...
  // Healing calculation for medical operators
  ...
  let skillHps, normalHps, cycleHps, totalHeal;

  if (isMedic && !isIncantationMedic) {
    // Pure medical: only healing, no damage
    skillAttacks = 0;
    skillTotalDamage = 0;
    skillDps = 0;
    cycleDps = null;
    ...
    if (isToggle || isPermanent) {
      ...
      if (spType === 'INCREASE_WHEN_ATTACK') {
        ...
        cycleDps = cycleTime > 0 ? ((attacksToCharge * normalHitDamage) + singleHitDamage) / cycleTime : 0;
      } else {
        const attacksDuringCharge = Math.floor(spCost / realInterval);
        cycleDps = spCost > 0 ? ((attacksDuringCharge * normalHitDamage) + singleHitDamage) / spCost : 0;
      }
    }
  } else {
    // Pure damage operator
  ...
  return { type: isMedic ? 'heal' : 'damage', skillDps, skillTotalDamage, cycleDps, normalDps, isToggle, isPermanent, realInterval, panelAtk: skillAtk, skillHps, normalHps, totalHeal };
}
